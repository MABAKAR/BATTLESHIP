...
(function() {
    ...
    var game = {
        ...
        // défini si c'est verticale ou pas
        isVerticale: false,
        ...
        gameIsOver: function() {
            ...
            this.players.forEach(element => {
                ...
                for(let y = 0; y < 10; y++){
                    for(let x = 0; x < 10; x++){
                        if(element.grid[y][x] > 0){
                            //incrémente isOver si le joueur a encore des bateaux
                            isOver++ ;
                            break loop1;
                        }
                    }
            ...
            if(isOver === 2){
                // si isOver = 2, ça signifie que les deux joueurs ont encore des bateaux, la partie continue, on retourne donc false
                return false ;
            }
            // La partie est terminé, on retourne true
            utils.info("La partie est terminé");
            return true;
        },
        getPhase: function() {
            if (this.waiting) {
        ...
        handleClick: function(e) {
            ...
            if (e.target.classList.contains('cell')) {
                ...
                if (this.getPhase() === this.PHASE_INIT_PLAYER) {
                    ...
                    if (this.players[0].setActiveShipPosition(utils.eq(e.target), utils.eq(e.target.parentNode), this.isVerticale)) {
        ...
        handleRightClick: function(e) {
            e.preventDefault();
            this.isVerticale = this.isVerticale ? false : true;
            var ship = this.players[0].fleet[this.players[0].activeShip];
            ship.dom.style.transform = "rotate(0deg)";
            if (this.isVerticale) {
                if (ship.getLife() % 2 === 0) {
        ...
        fire: function(from, col, line, callback) {
            ...
            target.receiveAttack(col, line, function(hasSucceed) {
                ...
                callback(hasSucceed);
                self.stopWaiting();
                if(hasSucceed && target !== self.players[1]){
                    self.players[1].renderHit(self.miniGrid);
                }
                self.renderMap();
        ...
        renderMiniMap: function() {
            ...
            this.players[0].grid.forEach(function(row, rid) {
                row.forEach(function(val, col) {
                    if (val !== 0) {
                        var obj = {};
                        self.players[0].fleet.forEach((ship) => {
                            if (val === ship.id) {
                                obj = ship;
                            }
                        })
                        var node = self.miniGrid.querySelector('.row:nth-child(' + (rid + 1) + ') .cell:nth-child(' + (col + 1) + ')');
                        node.style.backgroundColor = obj.getColor();
                    }
                });
            });
        }
